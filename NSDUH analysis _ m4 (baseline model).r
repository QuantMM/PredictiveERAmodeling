...
##########-------------------------------------------------
##########
# Model 4 (Baseline Model): Y = F1 + F2 + F3
##########
##########-------------------------------------------------
...
# 1-1. Train set:
	...
	X_t <- cbind(x1_t,x2_t,x3_t)

	nvar_t <- matrix(c(ncol(x1_t),ncol(x2_t),ncol(x3_t)), ncol=3)
...
# 1-2. Test set:
	...
	X_v <- cbind(x1_v,x2_v,x3_v)

	nvar_v <- matrix(c(ncol(x1_v),ncol(x2_v),ncol(x3_v)), ncol=3)
