...
##########-------------------------------------------------
##########
# Model 12: Y = F1 + F2 + F3 + F1*F2 + F1*F3 + F2*F3 + F1*F2*F3
##########
##########-------------------------------------------------
...
# 1-1. Train set:
	...
	int2 <- x2_t #second interaction component
	...
	rm(int1); rm(int2)
	inters_t3 <- inters; rm(inters)

	# Interaction term 4 (three-way) creation:
	
	int1 <- x1_t #first interaction component
	int2 <- x2_t #second interaction component
	int3 <- x3_t #third interaction component
		
		xpairs <- expand.grid(colnames(int1), colnames(int2), colnames(int3), stringsAsFactors=F)
		inters <- data.frame(matrix(, nrow= nrow(int1), ncol= nrow(xpairs)))
		...
		for (tt in 1:nrow(xpairs)) {
			...
			factor_1 <- xpairs[tt, 1]
			factor_2 <- xpairs[tt, 2]
			factor_3 <- xpairs[tt, 3]
			inters[,tt] <- int1[,colnames(int1) == factor_1] * int2[,colnames(int2) == factor_2] * int3[,colnames(int3) == factor_3]
			colnames(inters)[tt] <- gsub("\\.", ":", paste(factor_1, factor_2, factor_3, sep=":"))
		}

	rm(int1); rm(int2); rm(int3)
	inters_t4 <- inters; rm(inters)

	X_t <- cbind(x1_t,x2_t,x3_t,inters_t1,inters_t2,inters_t3,inters_t4)

	nvar_t <- matrix(c(ncol(x1_t),ncol(x2_t),ncol(x3_t),ncol(inters_t1),ncol(inters_t2),ncol(inters_t3),ncol(inters_t4)), ncol=7)
...
# 1-2. Test set:
	...
	int2 <- x2_v #second interaction component
	...
	rm(int1); rm(int2)
	inters_v3 <- inters; rm(inters)

	# Interaction term 4 (three-way) creation:
	
	int1 <- x1_v #first interaction component
	int2 <- x2_v #second interaction component
	int3 <- x3_v #third interaction component
	...
	rm(int1); rm(int2); rm(int3)
	inters_v4 <- inters; rm(inters)

	X_v <- cbind(x1_v,x2_v,x3_v,inters_v1,inters_v2,inters_v3,inters_v4)

	nvar_v <- matrix(c(ncol(x1_v),ncol(x2_v),ncol(x3_v),ncol(inters_v1),ncol(inters_v2),ncol(inters_v3),ncol(inters_v4)), ncol=7)
