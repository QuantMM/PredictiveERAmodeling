...
# 1-1. Train set:
	...
	rm(int1); rm(int2)
	inters_t <- inters; rm(inters)

	X_t <- cbind(x1_t,x2_t,x3_t,inters_t)

	nvar_t <- matrix(c(ncol(x1_t),ncol(x2_t),ncol(x3_t),ncol(inters_t)), ncol=4)
...
# 1-2. Test set:
	...
	rm(int1); rm(int2)
	inters_v <- inters; rm(inters)

	X_v <- cbind(x1_v,x2_v,x3_v,inters_v)

	nvar_v <- matrix(c(ncol(x1_v),ncol(x2_v),ncol(x3_v),ncol(inters_v)), ncol=4)
